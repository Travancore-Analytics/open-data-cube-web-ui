...
def pixel_drill(task_id=None):
    ...
    clear_mask = np.full(single_pixel[list(single_pixel.data_vars)[0]].shape, True)
    ...
    exclusion_list = ['satellite', 'quality']
        ...
        for index in range((task.get_chunk_size()['time'] if task.get_chunk_size()['time'] is not None else 1)):
            ...
            if os.path.exists(path):
                ...
                if task.animated_product.animation_id == "cumulative":
                    ...
                    clear_mask = np.full(animated_data[list(animated_data.data_vars)[0]].shape, True)
